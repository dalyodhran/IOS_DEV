...
class FCViewController: UIViewController, UINavigationControllerDelegate {
    ...
    func fetchConfig() {
        ...
        remoteConfig.fetch(withExpirationDuration: expirationDuration){ (status, error) in
            if status == .success {
                ...
                let friendlyMsgLength = self.remoteConfig["friendly_meg_length"]
    ...
    func sendPhotoMessage(photoData: Data) {
        ...
        storageRef!.child(imagePath).putData(photoData, metadata: metadata) { (metadata, error) in
            ...
            self.sendMessage(data: [Constants.MessageFields.imageUrl: String(self.storageRef!.child((metadata?.path)!).description)])
        }
    }
...
extension FCViewController: UIImagePickerControllerDelegate {
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [String:Any]) {
        ...
        if let photo = info[UIImagePickerControllerOriginalImage] as? UIImage, let photoData =
            UIImageJPEGRepresentation(photo, 0.8) {
